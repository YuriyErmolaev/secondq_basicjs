...
document.querySelectorAll('button').forEach((elButton)=>{
...
function clickHandler(event) {
    ...
    let elButton = event.target,
        productsType =  elButton.dataset.type;
    showCategory(productsType);
}
...
function showCategory(category) {
    let categoryAr = products[category];
    categoryAr.forEach((product)=>{
       let productElHtml = getProductMarkup(product);
        productsContainer.insertAdjacentHTML('afterbegin', productElHtml);
    });
}
